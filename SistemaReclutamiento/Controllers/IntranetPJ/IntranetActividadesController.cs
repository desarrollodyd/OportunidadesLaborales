...
namespace SistemaReclutamiento.Controllers.IntranetPJ
{
    public class IntranetActividadesController : Controller
    {
        ...
        [HttpPost]
        public ActionResult IntranetActividadesEliminarJson(int Actividades_id)
        {
            string errormensaje = "";
            bool respuestaConsulta = false;
            string mensajeConsola = "";
            try
            {
                var ActividadesTupla = intranetActividadesbl.IntranetActividadesEliminarJson(Actividades_id);
                error = ActividadesTupla.error;
                if (error.Key.Equals(string.Empty))
                {
